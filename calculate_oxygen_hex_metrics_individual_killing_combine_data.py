...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr 13 18:46:31 2024

@author: Vedang Narain (vedang.narain@msdtc.ox.ac.uk)

Calculate metrics for the hexagonal network with log-normally distributed radii and individual vessel pruning and oxygenation.
...
def get_simulation_stats(which_layout, mu, sigma, n_kills, folder_path_pre_kill_number, network_file_name, network_path, flat_field, field_data):    
...
# Define a function to return statistics for a single network layout with different kills (FAKE DATASET)
def get_layout_stats(which_layout):

    layout_folder_path = os.path.join(main_folder_path, solver_name + 'Haematocrit/Selection' + str(which_layout))

    # Initialise empty dfs to store simulations for the whole layout
    layout_df = pd.DataFrame()
    killed_vessels_df = pd.DataFrame()

    # List all sigma directories in the selection folder
    sigma_dirs = [entry for entry in os.listdir(layout_folder_path) if entry.startswith('Sigma') and os.path.isdir(os.path.join(layout_folder_path, entry))]
    
    # Sort the Sigma directories in ascending order based on their numeric value
    sigma_dirs.sort(key=lambda x: float(x.replace('Sigma', '')))
    
    # Iterate over all Sigma directories
    for sigma in sigma_dirs:
        sigma_folder_path = os.path.join(layout_folder_path, sigma)

        # List all Mu directories within the Sigma folder
        mu_dirs = [entry for entry in os.listdir(sigma_folder_path) if entry.startswith('Mu') and os.path.isdir(os.path.join(sigma_folder_path, entry))]
        
        # Sort the Mu directories in ascending order based on their numeric value
        mu_dirs.sort(key=lambda x: float(x.replace('Mu', '')))
        
        # Iterate over all Mu directories
        for mu in mu_dirs:
            mu_folder_path = os.path.join(sigma_folder_path, mu)

            # List all Kills directories within the Mu folder
            kills_dirs = [entry for entry in os.listdir(mu_folder_path) if entry.startswith('Kills') and os.path.isdir(os.path.join(mu_folder_path, entry))]
            
            # Sort the Kills directories in ascending order based on their numeric value
            kills_dirs.sort(key=lambda x: int(x.replace('Kills', '')))
            ...
            for kills in kills_dirs:
                ...
                # Set the file path
                n_kills = int(kills[5:])
                kills_folder_path = os.path.join(mu_folder_path, kills)
                folder_path_pre_kill_number = os.path.join(mu_folder_path, kills[:5])
                ...
                if np.sum(flat_field) < 0.000001:
                    print(f'No oxygen detected for Mu: {mu}, Kills: {kills}. Skipping remaining kills in this Mu.')
                    
                    # Perform the necessary logging before skipping
                    simulation_df, edge_metrics_df, reference_node_coordinates, reference_network_vessel_nodes = get_simulation_stats(
                        which_layout, mu, sigma, n_kills, folder_path_pre_kill_number, network_file_name, network_path, flat_field, field_data
                    )
                    ...
                    if int(n_kills) > 0:
                        ...
                        killed_vessel_df = log_killed_vessel(
                            network_path, previous_network_path, previous_edge_metrics_df, reference_node_coordinates, reference_network_vessel_nodes
                        )
                        killed_vessel_df.insert(0, 'kills', n_kills)
                        killed_vessel_df.insert(0, 'mu', mu)
                        killed_vessel_df.insert(0, 'sigma', sigma)
                    ...
                    break  # Skip to the next Mu after this kills is processed
                
                # Proceed with normal processing if oxygen is detected
                simulation_df, edge_metrics_df, reference_node_coordinates, reference_network_vessel_nodes = get_simulation_stats(
                    which_layout, mu, sigma, n_kills, folder_path_pre_kill_number, network_file_name, network_path, flat_field, field_data
                )
                layout_df = pd.concat([layout_df, simulation_df], ignore_index=True)
...
# Toggles
vessel_length = 85  # in micrometres
max_layout = 100
...
# hypoxic_threshold_list = [4116, 10975]  # 3 mmHg and 8 mmHg
...
# inlet_radius_m = 1.500000e-05
inlet_h = 0.3
pq_threshold = 3.e-14
# main_folder_path = '/scratch/narain/Hexagonal/85 VesselLength/no_rt_size_pruning/TestMantegazzaNetwork/'
# save_path = '/home/narain/Desktop/Scripts/demo_datasets/hex_size/raw_hex_size.h5'  # Specify the file path
main_folder_path = '/scratch/narain/Hexagonal/85 VesselLength/no_rt_flow_pruning/TestMantegazzaNetwork/'
save_path = '/home/narain/Desktop/Scripts/demo_datasets/hex_flow/raw_hex_flow.h5'  # Specify the file path

# =============================================================================
# THIS CODE IS FOR CREATING A DATASET FOR PREDICTIONS
# =============================================================================
# '''
